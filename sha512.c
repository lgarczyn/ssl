...
#define RR(val, shift)		(((val) >> (shift)) | ((val) << (64 - (shift))))

#define MAJ(x, y, z) (((x) & (y)) | ((z) & ((x) | (y))))
#define CH(x, y, z) ((z) ^ ((x) & ((y) ^ (z))))

#define EP0(x) (RR((x), 28) ^ RR((x), 34) ^ RR((x), 39))
#define EP1(x) (RR((x), 14) ^ RR((x), 18) ^ RR((x), 41))
...
void			module_sha512(t_args *args, t_file *file)
{
	...
	t_ulong		vars[SHA512_VARS];
	t_ulong		tmp_vars[SHA512_VARS];
	int			i;
